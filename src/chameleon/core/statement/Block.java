 ...
 */
public class Block extends StatementImpl<Block> implements
		StatementListContainer<Block> {
	// TODO: should this be a member, or should there be a separate
	// ObjectInitializer that contains a block ?
	// TODO: can any statement be a member ? In this case the methods of Member
	// have to move up.

	/**
	 * STATEMENTS
	 */
	private OrderedMultiAssociation<Block, Statement> _statements = new OrderedMultiAssociation<Block, Statement>(
			this);
	 ...
	 * @
	 * 
	 * @ post getStatements().indexOf(element) == 0) ==> \result ==
	 * parent().lexicalContext(this);
	 * 
	 * @ post getStatements().indexOf(Element) > 0) ==>
	 * 
	 * @ \result == getStatements().elementAt(getStatements().indexOf(element) -
	 * 1).lexicalContext();
	 * 
	 * @
	 */
	public LookupStrategy lexicalLookupStrategy(Element element)
		...
		if (index == 0) {
			...
			throw new ChameleonProgrammerException(
					"Invoking lexicalContext(element) with an element that is not a child.");
		}
	}
